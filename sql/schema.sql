...
CREATE TABLE task (
  ...
  after_complete_rule  json                                        DEFAULT NULL COMMENT '完成后的后续规则（JSON格式）',
  complete_engage_way  smallint(6)                                 DEFAULT NULL COMMENT '完成后的触达方式，取值：1-PUSH 2-弹窗',
  complete_engage_rule json                                        DEFAULT NULL COMMENT '完成后的触达规则（JSON格式）',
  button_config        json                               NOT NULL COMMENT '按钮配置（JSON格式）',
  redirect_link_config json                               NOT NULL COMMENT '跳转链接配置（JSON格式）',
  PRIMARY KEY (id)
...
CREATE TABLE trigger_log (
  ...
  event_rule_data      json                                        DEFAULT NULL COMMENT '事件规则数据',
  event_time           datetime                           NOT NULL COMMENT '事件完成的时间',
...
CREATE TABLE complete_record (
  ...
  after_complete_rule  json                                        COMMENT '完成后的后续规则（JSON格式）',
  PRIMARY KEY (id),
  KEY idx_user_id (user_id),
  KEY idx_task_id (task_id),
